...
class LLMClient:
    """Minimal LLM client using HTTP for Gemini API.

    Supports GEMINI provider via env:
    ...
    def _gemini_chat(self, prompt: str, temperature: float = 0.1) -> str:
    ...
    def analyze_symptoms(self, symptoms: str, age: str, locale: str = "en-IN") -> Dict[str, Any]:
        prompt = f"""You are a safe medical triage assistant for India. Respond in strict JSON format only.

Patient symptoms: {symptoms}. Age group: {age}.

Return a JSON object with these exact keys:
- triage_level: one of "emergency", "urgent", "routine", "self_care"
- assessment: brief assessment of symptoms
- otc_medicines: array of objects with name, dose, notes (only OTC medicines available in India)
- home_remedies: array of strings with Indian home remedies
- warning_signs: array of strings with warning signs to watch
- follow_up: follow-up advice
- red_flags: array of any red flags detected

Never include prescription-only (Schedule H) drugs. Use only adult-safe OTC medicines.
Keep responses concise and practical for Indian context.

JSON response:"""

        content = self._gemini_chat(prompt)
        return self._parse_json(content)

    def suggest_medicine(self, condition: str, age: str) -> Dict[str, Any]:
        prompt = f"""You suggest only OTC medicines available in India. Respond in strict JSON format only.

Condition: {condition}. Age group: {age}.

Return a JSON object with these exact keys:
- medicines: array of objects with name, dose, frequency, max_daily, notes
- general_advice: general advice about the condition
- disclaimers: array of safety disclaimers

Allowed medicines: Paracetamol, Ibuprofen, ORS, Antacid, Cetirizine, Loratadine.
...
JSON response:"""
    ...
    def home_remedies(self, condition: str) -> Dict[str, Any]:
        prompt = f"""You provide safe, culturally appropriate Indian home remedies. Respond in strict JSON format only.

Condition: {condition}.

Return a JSON object with these exact keys:
- remedies: array of strings with home remedies
- general_tips: array of strings with general tips
- warnings: array of strings with warnings

Focus on Indian cultural context and easily available ingredients.
...
class MedicalMCPServer:
    ...
    def analyze_symptoms(self, symptoms: str, age: str = "adult", location: str = None) -> Dict[str, Any]:
        ...
        # Prefer LLM if configured; fallback to rule-based
        if self.llm.is_enabled():
            try:
                llm_resp = self.llm.analyze_symptoms(symptoms, age)
                # Derive condition heuristically for warning signs
                condition = self._identify_condition(symptoms_lower)
                triage_level = llm_resp.get("triage_level", "self_care")
                # Safety post-filtering
                llm_resp["otc_medicines"] = self._filter_otc_list(
                    llm_resp.get("otc_medicines", []))
                result = {
                    "triage_level": triage_level,
                    "condition": condition,
                    "assessment": llm_resp.get("assessment", f"Based on symptoms: {symptoms}"),
                    "otc_medicines": llm_resp.get("otc_medicines", []),
                    "home_remedies": llm_resp.get("home_remedies", []),
                    "follow_up": llm_resp.get("follow_up", self._get_follow_up_advice(triage_level)),
                    "warning_signs": llm_resp.get("warning_signs", self._get_warning_signs(condition)),
                    "red_flags": llm_resp.get("red_flags", []),
                    "disclaimer": "⚠️ This is informational only. Consult a healthcare professional for medical advice."
                }
            except Exception as e:
                # Fallback to static logic on LLM failure
                condition = self._identify_condition(symptoms_lower)
                triage_level = self._determine_triage_level(
                    symptoms_lower, condition)
    ...
    def suggest_medicine(self, condition: str, age: str = "adult") -> Dict[str, Any]:
        ...
        if self.llm.is_enabled():
            try:
                llm_resp = self.llm.suggest_medicine(condition, age)
                meds = self._filter_otc_list(llm_resp.get("medicines", []))
                result = {
                    "condition": condition,
                    "medicines": meds,
                    "general_advice": llm_resp.get("general_advice", "Only use medicines as directed on the package"),
    ...
    def get_home_remedies(self, condition: str) -> Dict[str, Any]:
        ...
        if self.llm.is_enabled():
            try:
                llm_resp = self.llm.home_remedies(condition)
                result = {
                    "condition": condition,
                    "remedies": llm_resp.get("remedies", []),
                    "general_tips": llm_resp.get("general_tips", []),
                    "warnings": llm_resp.get("warnings", []),
        ...
        try:
            ...
            for place in places_result.get('results', [])[:5]:
                ...
                if 'geometry' in place:
                    ...
                    chemist['distance_km'] = "Calculating..."
    ...
    def _filter_otc_list(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Filter LLM-proposed medicines to OTC-only by simple rules."""
        allowed = {
            "paracetamol",
            "acetaminophen",
            "ibuprofen",
        ...
        forbidden_keywords = {
            "antibiotic", "steroid", "schedule h", "rx", "prescription"
        }
        filtered: List[Dict[str, Any]] = []
        for item in items:
            name = str(item.get("name", "")).lower()
            if any(k in name for k in forbidden_keywords):
                continue
            if allowed and not any(a in name for a in allowed):
                # Skip unknowns to be safe
                continue
